...
#define __PLOG_PROCESS_KEY_ELEMENT(r, unused, idx, elem)                       \
  BOOST_PP_EXPR_IF(idx, " ") BOOST_PP_TUPLE_ELEM(2, 0, elem) ":{}"
#define __PLOG_PROCESS_VALUE_ELEMENT(r, unused, idx, elem)                     \
  BOOST_PP_COMMA_IF(idx) BOOST_PP_TUPLE_ELEM(2, 1, elem)
...
#define __PLOG(LEVEL, seq)                                                     \
  do {                                                                         \
    ...
    SPDLOG_##LEVEL(fmt,                                                        \
                   BOOST_PP_SEQ_FOR_EACH_I(__PLOG_PROCESS_VALUE_ELEMENT, % %,  \
                                           __PLOG_PREPROCESS_PAIRS(seq)));     \
  } while (0)
